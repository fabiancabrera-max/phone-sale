...
const VALID_CONDITIONS: ProductCondition[] = ['new', 'used_as_new', 'used', 'refurbished'];
...
export function validateCreateProduct(input: unknown): CreateProductInput {
    ...
    if (typeof data.title !== 'string') {
        throw new ValidationError('Title is required and must be a string', ERROR_CODES.INVALID_INPUT, 'title');
    }
    ...
    if (typeof data.description !== 'string') {
        throw new ValidationError('Description is required and must be a string', ERROR_CODES.INVALID_INPUT, 'description');
    }
    ...
    if (typeof data.price !== 'number') {
        throw new ValidationError('Price is required and must be a number', ERROR_CODES.INVALID_INPUT, 'price');
    }
    ...
    if (!data.specs || typeof data.specs !== 'object') {
        throw new ValidationError('Specs object is required', ERROR_CODES.INVALID_INPUT, 'specs');
    }

    // Validate specs.condition
    if (typeof data.specs.condition !== 'string' || !VALID_CONDITIONS.includes(data.specs.condition)) {
        throw new ValidationError(
    ...
    if (typeof data.specs.color !== 'string' || data.specs.color.trim().length === 0) {
        throw new ValidationError('Color is required and must be a non-empty string', ERROR_CODES.INVALID_INPUT, 'specs.color');
    }

    // Validate specs.storage
    if (typeof data.specs.storage !== 'string' || data.specs.storage.trim().length === 0) {
        throw new ValidationError('Storage is required and must be a non-empty string', ERROR_CODES.INVALID_INPUT, 'specs.storage');
    }

    // Validate images
    if (!Array.isArray(data.images)) {
        throw new ValidationError('Images must be an array', ERROR_CODES.INVALID_INPUT, 'images');
    }
    if (data.images.length === 0) {
        throw new ValidationError('At least one image is required', ERROR_CODES.INVALID_INPUT, 'images');
    }
    ...
    if (!data.images.every((img: any) => typeof img === 'string' && img.startsWith('http'))) {
        throw new ValidationError('All images must be valid URLs', ERROR_CODES.INVALID_INPUT, 'images');
    }

    // Validate status (optional)
    const validStatuses: ProductStatus[] = ['featured', 'on_sale', 'sold'];
    if (data.status !== undefined) {
        if (typeof data.status !== 'string' || !validStatuses.includes(data.status as ProductStatus)) {
            throw new ValidationError(
...
export function validateUpdateProduct(input: unknown): UpdateProductInput {
    ...
    if (data.title !== undefined) {
        if (typeof data.title !== 'string') {
            throw new ValidationError('Title must be a string', ERROR_CODES.INVALID_INPUT, 'title');
        }
        if (data.title.length < PRODUCT_VALIDATION.MIN_TITLE_LENGTH) {
            throw new ValidationError(
                `Title must be at least ${PRODUCT_VALIDATION.MIN_TITLE_LENGTH} characters`,
    ...
    if (data.description !== undefined) {
        if (typeof data.description !== 'string') {
            throw new ValidationError('Description must be a string', ERROR_CODES.INVALID_INPUT, 'description');
        }
        if (data.description.length < PRODUCT_VALIDATION.MIN_DESCRIPTION_LENGTH) {
            throw new ValidationError(
                `Description must be at least ${PRODUCT_VALIDATION.MIN_DESCRIPTION_LENGTH} characters`,
    ...
    if (data.price !== undefined) {
        if (typeof data.price !== 'number') {
            throw new ValidationError('Price must be a number', ERROR_CODES.INVALID_INPUT, 'price');
        }
        if (data.price < PRODUCT_VALIDATION.MIN_PRICE || data.price > PRODUCT_VALIDATION.MAX_PRICE) {
            throw new ValidationError(
    ...
    if (data.specs !== undefined) {
        if (typeof data.specs !== 'object') {
            throw new ValidationError('Specs must be an object', ERROR_CODES.INVALID_INPUT, 'specs');
        }
        ...
        if (data.specs.color !== undefined) {
            if (typeof data.specs.color !== 'string' || data.specs.color.trim().length === 0) {
                throw new ValidationError('Color must be a non-empty string', ERROR_CODES.INVALID_INPUT, 'specs.color');
            }
        ...
        if (data.specs.storage !== undefined) {
            if (typeof data.specs.storage !== 'string' || data.specs.storage.trim().length === 0) {
                throw new ValidationError('Storage must be a non-empty string', ERROR_CODES.INVALID_INPUT, 'specs.storage');
            }
